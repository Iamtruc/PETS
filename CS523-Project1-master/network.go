...
type TCPNetworkStruct struct {
	// We map all the connections and have a mutex to avoid collisions.
...
// We connect according to ID Numbers.
...
// We listen for connections ans when there is one, we intercept it.
	go func() {
...
// Once we have taken all the waitFor connections, we compute the askFor connections.
